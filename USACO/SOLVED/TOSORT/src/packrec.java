...
public class packrec {
    ...
    public static void secondConfig(Rectangle[] rct) {
        int length = Math.max(Math.max(rct[0].l, rct[1].l), rct[2].l) + rct[3].l;
        int width = Math.max(rct[0].w + rct[1].w + rct[2].w, rct[3].w);
        if (length * width < minArea) {
            minArea = length * width;
            soln.clear();
            soln.add(new Rectangle(Math.min(length, width), Math.max(length, width)));
    ...
    public static void thirdConfig(Rectangle[] rct) {
        int length = 0;
        int width = 0;
        width = Math.max(rct[0].w + rct[1].w, rct[2].w) + rct[3].w;
        length = Math.max(Math.max(rct[0].l, rct[1].l) + rct[2].l, rct[3].l);
        if (length * width < minArea) {
            minArea = length * width;
            soln.clear();
            soln.add(new Rectangle(Math.min(length, width), Math.max(length, width)));
    ...
    public static void fourthConfig(Rectangle[] rct) {
        int length = 0;
        int width = 0;
        length = Math.max(rct[0].l, rct[3].l);
        length = Math.max(length, rct[1].l + rct[2].l);
        length = Math.max(length, rct[1].l + rct[0].l);
        length = Math.max(length, rct[1].l + rct[3].l);
        length = Math.max(length, rct[2].l + rct[0].l);
        length = Math.max(length, rct[2].l + rct[3].l);
        width = rct[0].w + Math.max(rct[1].w, rct[2].w) + rct[3].w;
        if (length * width < minArea) {
            minArea = length * width;
            soln.clear();
            soln.add(new Rectangle(Math.min(length, width), Math.max(length, width)));
    ...
    public static void fifthConfig(Rectangle[] rct) {
        int length = 0;
        int width = 0;
        length = rct[0].l + rct[1].l;
        length = Math.max(length, rct[2].l);
        length = Math.max(length, rct[3].l);
        width = Math.max(rct[0].w, rct[1].w) + rct[2].w + rct[3].w;
        if (length * width < minArea) {
            minArea = length * width;
            soln.clear();
    ...
    public static void sixthConfig(Rectangle[] rct) {
        int length = 0;
        int width = 0;
        length = Math.max(rct[0].l + rct[1].l, rct[2].l + rct[3].l);
        width = rct[1].w + rct[3].w;
        if (rct[1].l < rct[3].l) {//check different width combinations (they dont always happen)
            width = Math.max(width, rct[0].w + rct[3].w);
        }
        if (rct[3].l < rct[1].l) {
            width = Math.max(width, rct[1].w + rct[2].w);
        }
        if (rct[3].l <= rct[0].l + rct[1].l) {
            width = Math.max(width, rct[0].w + rct[2].w);
        } else {
            width = Math.max(width, rct[2].w);
        }
        if (rct[1].l > rct[2].l + rct[3].l) {
            width = Math.max(width, rct[0].w);
        }
        if (length * width < minArea) {
            minArea = length * width;
            soln.clear();
